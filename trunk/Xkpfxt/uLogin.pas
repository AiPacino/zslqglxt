...
uses
  ...
  CnAAFont, CnAACtrls, auHTTP, pngimage, frxpngimage, Mask, DBCtrlsEh, jpeg;
...
begin
  ...
  if not Result then
  begin
    MessageBox(Application.Handle,PChar('数据库连接失败！　'),pchar('系统提示'),MB_OK+MB_ICONERROR);
  end;
end;
...
begin
  ...
  try
    ...
    begin
      inc(iCount);
      //=============考生是否合法==============//
      if dm.CzyLogin(Edit1.Text,Edit2.Text,Get_Version,sMsg) = False then
      begin
        gbIsOK := false;
      ...
      begin
        ...
        gb_Czy_NAME := dm.GetCzyName(gb_Czy_ID);
        gb_Czy_Level := dm.GetCzyLevel(gb_Czy_ID);
        gb_czy_Dept := dm.GetCzyDept(gb_Czy_Id);
        gbIsOK := True;
{
...
begin
  try
    dm.con_DB.Close;
    dm.con_DB.ConnectionTimeout := 3;
    dm.con_DB.ConnectionString := dm.GetConnInfo;
    dm.con_DB.Connected := True;
    dm.con_DB.ConnectionTimeout := 10;
    Application.MessageBox('操作完成！数据库连接成功！', '系统提示', MB_OK + 
      MB_ICONINFORMATION + MB_TOPMOST);
    dm.con_DB.Close;
  except
    Application.MessageBox('数据库连接失败！请检查后重试！', '系统提示', MB_OK
