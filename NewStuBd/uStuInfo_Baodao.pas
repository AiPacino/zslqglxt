...
procedure TStuInfo_Baodao.DataSource1DataChange(Sender: TObject; Field: TField);
var
...
function TStuInfo_Baodao.DownLoadPhotoFromUrl(const Photo_Url: string; aImage: TImage;
...
procedure TStuInfo_Baodao.FormCreate(Sender: TObject);
begin
  sWebSrvUrl := dm.GetWebSrvUrl;
end;

procedure TStuInfo_Baodao.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
    VK_UP:
...
procedure TStuInfo_Baodao.FormShow(Sender: TObject);
var
  ksh,fn,sPath:string;
begin
  Ksh := DataSource1.DataSet.FieldByName('考生号').AsString;
...
procedure TStuInfo_Baodao.N1Click(Sender: TObject);
begin
