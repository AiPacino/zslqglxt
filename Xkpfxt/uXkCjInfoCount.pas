...
uses
  ...
  DBGridEhGrouping, Mask, DBCtrlsEh;

type
  TXkCjInfoCount = class(TForm)
...
var
  XkCjInfoCount: TXkCjInfoCount;

implementation
uses uDM;
{$R *.dfm}

procedure TXkCjInfoCount.BitBtn1Click(Sender: TObject);
begin
...
procedure TXkCjInfoCount.btn_ExcelClick(Sender: TObject);
begin
  dm.ExportDBEditEH(dxgrd_1);
end;

procedure TXkCjInfoCount.btn_ExitClick(Sender: TObject);
begin
  Close;
end;

procedure TXkCjInfoCount.btn_PrintClick(Sender: TObject);
begin
  PrintDBGridEH(dxgrd_1,Self,RzRadioGroup1.Items[RzRadioGroup1.ItemIndex]);
end;

procedure TXkCjInfoCount.btn_RefreshBdlClick(Sender: TObject);
begin
  mi_RefreshBdl.Click;
end;

procedure TXkCjInfoCount.btn_RefreshClick(Sender: TObject);
begin
  Open_Table;
end;

procedure TXkCjInfoCount.cbb_YxChange(Sender: TObject);
begin
...
procedure TXkCjInfoCount.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TXkCjInfoCount.FormCreate(Sender: TObject);
begin
...
procedure TXkCjInfoCount.FormDestroy(Sender: TObject);
begin
  FreeAndNil(sqlList);
end;

procedure TXkCjInfoCount.FormShow(Sender: TObject);
begin
  if RzRadioGroup1.Items.Count>0 then
    RzRadioGroup1.ItemIndex := 0;
end;

procedure TXkCjInfoCount.GetYxList;
var
  sList:TStrings;
begin
...
procedure TXkCjInfoCount.mi_RefreshBdlClick(Sender: TObject);
begin
  RzRadioGroup1.OnClick(Self);
end;

procedure TXkCjInfoCount.Open_Table;
var
  cds_Temp:TClientDataSet;
begin
  cds_Temp := TClientDataSet.Create(nil);
  try
    cds_Temp.XMLData := dm.OpenData('select * from SqlCmd配置总表 where 模块='+quotedstr(gb_System_Mode)+' and 分类='+quotedstr('成绩'));
    RzRadioGroup1.Items.Clear;
    sqlList.Clear;
...
procedure TXkCjInfoCount.RzRadioGroup1Click(Sender: TObject);
var
